...
class GRPO(Agent):
    def __init__(
        ...
        """
        _cfg = copy.deepcopy(GRPO_DEFAULT_CONFIG)
    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        ...
        def compute_ra(
            rewards: torch.Tensor,  # (16, 4096, 1)
            dones: torch.Tensor,
            ...
            :rtype: torch.Tensor
            """
            ## print(rewards.shape, dones.shape, values.shape, next_values.shape)
            advantage = 0
            ...
            for i in reversed(range(memory_size)):
                ...
                ## advantage = (
                ##     rewards[i] - values[i] + next_values * discount_factor * not_dones[i]  # TD residual
                ##     + discount_factor * not_dones[i] * lambda_coefficient * advantage      # lambda * next_advantage
                ## )
                ### GRPO
                advantage = rewards[i] + discount_factor * advantage
        ...
        for epoch in range(self._learning_epochs):
            ...
            for (
                ...
                ### print("sampled_log_prob.shape: ", sampled_log_prob.shape)
                ### print("sampled_values.shape: ", sampled_values.shape)
                ### print("sampled_returns.shape: ", sampled_returns.shape)
                ### print("sampled_advantages.shape: ", sampled_advantages.shape)
